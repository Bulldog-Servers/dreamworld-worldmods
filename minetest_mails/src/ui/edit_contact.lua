...
local FORMNAME = "minetest_email:editcontact"
...
function mail.show_edit_contact(name, contact_name, note, illegal_name_hint)
	...
	if illegal_name_hint == "collision" then
		formspec = formspec .. [[
				textarea[4.2,1.2;2,2;;;]] .. S("That name is already in your contacts.") .. [[]
			]]
	elseif illegal_name_hint == "empty" then
		formspec = formspec .. [[
				textarea[4.2,1.2;2,2;;;]] .. S("The contact name cannot be empty.") .. [[]
			]]
	end
